...
# Calculating response time by station for catual fires.
...
dispatch_date_time = data |>
  ...
  average_response_time_by_station = dispatch_date_time_fires |>
    group_by(station) |>
    summarize(average_response_time = mean(response_time, na.rm=T))
  
  highest_response_time_station = average_response_time_by_station |>
  filter(average_response_time == max(average_response_time, na.rm=T))
  
lowest_response_time_station = average_response_time_by_station |>
  filter(average_response_time == min(average_response_time, na.rm=T))

average_response_time_by_station
highest_response_time_station
lowest_response_time_station

```


# Determine if wake forest county fire response times have been going up or down for actual fires.
# determine the reson behind the changes.
